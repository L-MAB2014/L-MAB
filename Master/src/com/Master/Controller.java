...
public class Controller implements IController, IStockInput {
    ...
    /**
     * Enthält alle Checkpoints des Lagers
     */
    private HashMap<String, Checkpoint> checkpoints;
    
    /**
     * Enthält alle Eingänge des Lagers
     */
    private Map<String, Stock> stocks;
    
    /**
     * EnthÃ¼llt alle Ausgänge des Lagers
     */
    ...
    Controller() {
        ...
        logger.info("Oberflaeche wird geöffnet und und die Actionlistener gesetzt");
    ...
    public void InputConsole(String text) {
    	logger.info("Der Text  "+ text + " wurde in die Console eingefügt");
    ...
    public synchronized boolean CheckForContinue(String checkpoint, String next_checkpoint, Bot bot) {
        ...
        if (check != null) {
            ...
            if (checknext != null && checknext.getName().equals(next_checkpoint)) {
                ...
                logger.info("Der nächste Checkpunkt von "+ bot.getBt_Name() + " (" + next_checkpoint + ") ist nicht vorhanden");
            }
    ...
    /**
     * Überprüft ob eine Bot von einem zum nächsten Checkpunkt fahren darf oder nciht
    ...
    public synchronized boolean ExitReserved(String exit, Bot bot) {
        ...
        if(ex != null)
        {
        	...
        	logger.info("Ausgang erwartet als nächstes "+ nextID+" und der Bot hat  "+bot.getOrderID() +" geladen");
        	...
        	{
        		logger.info("Checkpoint "+ exit+" konnte nicht reserviert werden, da "+ex.getNextDelivery()+" erwartet wird und nicht "+bot.getOrderID()+" als nächstes");
        	}
        }else
        {
    ...
    public synchronized boolean StoreReserved(String store, Bot bot) {
        ...
        if(ex != null)
        {
        	...
        	{
        		logger.info("Checkpoint "+ store+" konnte nicht reserviert werden, da "+ex.getNextDelivery()+" erwartet wird und nicht "+bot.getOrderID()+" als nächstes");
        	}
        }else
        {
    ...
    /**
     * ActionListener zum Betätigen des Menüeintrags "Starten"
     */
    ...
    /**
     * ActionListener zum BetÃ¤tigen des MenÃ¼teintrags "Ã–ffnen"
     */
    ...
    /**
     * ActionListener zum Betätigen des Menüeintrags "Speichern"
     */
    ...
    /**
     * ActionListener zum Betätigen des Menüeintrags "Einstellungen"
     */
    ...
    /**
     * Klasse welche die Warteschlange eines Checkpunktes händelt
     */
            ...
            while (check != null && check.isBotInWaitList() && b) {
                ...
                if(check.isStore() || check.isExit())
                {
                	...
                	if(check.isStore())
                	{
                		...
                		logger.info("WorkOffWaitList Checkpunkt "+check.getName()+" artikel "+id+" muss als nächstes abgeholt werden");
                		for(int i = 0; (i < checkWaitList.size()) && (waitBot == null); ++i)
                		{
                			...
                			{
                				logger.info("WorkOffWaitList Checkpunkt "+check.getName()+" Bot "+ bot.getBt_Name()+" hat den Auftrag für Artikel "+id);
                				waitBot = bot;
                				checkWaitList.remove(bot);
                			}else
                			{
                				logger.info("WorkOffWaitList Checkpunkt "+check.getName()+" Bot "+ bot.getBt_Name()+" hat NICHT den Auftrag für Artikel "+id);
                			}
                		}
                	}else if(check.isExit())
                	{
                		...
                		logger.info("WorkOffWaitList Checkpunkt "+check.getName()+" artikel "+id+" muss als nächstes geliefert werden");
     ...
     * @author Marcel Reich
     * 
     * Klasse zum vergleichen der Eingänge (aus welchen Eingang als nächstes eine Order abgeholt werden solte)
     *
     */
